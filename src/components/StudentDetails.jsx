...
const StudentDetails = ({ data }) => {
  ...
  return (
    <div className=''>
      ...
      <p>Matriculation passing year: {student.matriculation_passing_year || "N/A"}</p>
      <p>Matriculation percentage: {student.matriculation_percentage || "N/A"}</p>
      <p>Matriculation board : {student.matriculation_board || "N/A"}</p>
      <p>Intermediate passing year: {student.intermediate_passing_year || "N/A"}</p>
      ...
      {student.linkedin_profile && (
        <a href={student.linkedin_profile} target="_blank" rel="noopener noreferrer">
          <button className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded'>LinkedIn</button>
        </a>
      )}
      </div>
    </div>
